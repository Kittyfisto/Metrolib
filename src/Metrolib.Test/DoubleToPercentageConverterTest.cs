...
namespace Metrolib.Test
{
	...
	public sealed class DoubleToPercentageConverterTest
	{
		...
		{
			new Action(() => _converter.Convert(null, null, null, null)).Should().NotThrow();
			new Action(() => _converter.Convert("", null, null, null)).Should().NotThrow();
			new Action(() => _converter.Convert(42, null, null, null)).Should().NotThrow();
			new Action(() => _converter.Convert("42", null, null, null)).Should().NotThrow();
			new Action(() => _converter.Convert(typeof(int), null, null, null)).Should().NotThrow();
		}
