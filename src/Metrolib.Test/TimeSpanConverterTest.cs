...
namespace Metrolib.Test
{
	...
	public sealed class TimeSpanConverterTest
	{
		...
		{
			new Action(() => _converter.Convert(null, typeof(string), null, CultureInfo.InvariantCulture)).Should().NotThrow();
			new Action(() => _converter.Convert(42, typeof(string), null, CultureInfo.InvariantCulture)).Should().NotThrow();
			new Action(() => _converter.Convert("foobar", typeof(string), null, CultureInfo.InvariantCulture)).Should().NotThrow();
		}
	}
}