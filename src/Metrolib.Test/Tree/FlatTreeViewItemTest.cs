...
namespace Metrolib.Test.Tree
{
	...
	public sealed class FlatTreeViewItemTest
	{
		...
		{
			var item = new FlatTreeViewItem();
			new Action(() => item.IsExpanded = true).Should().NotThrow();
			item.IsExpanded.Should().BeTrue();
			new Action(() => item.IsExpanded = false).Should().NotThrow();
		...
		{
			var item = new FlatTreeViewItem();
			new Action(() => item.IsSelected = true).Should().NotThrow();
			item.IsSelected.Should().BeTrue();
			new Action(() => item.IsSelected = false).Should().NotThrow();
