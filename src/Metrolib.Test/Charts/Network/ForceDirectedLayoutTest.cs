...
namespace Metrolib.Test.Charts.Network
{
	...
	public sealed class ForceDirectedLayoutTest
	{
		...
		public void TestRepulsiveness()
		{
			...
			changes.Should().Equal(new string[] {"Repulsiveness"});

			layout.Repulsiveness = 1001;
			changes.Should().Equal(new string[] { "Repulsiveness" });

			layout.Repulsiveness = 999;
			changes.Should().Equal(new string[] { "Repulsiveness", "Repulsiveness" });
		}
		...
		public void TestSpringStiffness()
		{
			...
			changes.Should().Equal(new string[] { "SpringStiffness" });

			layout.SpringStiffness = 1001;
			changes.Should().Equal(new string[] { "SpringStiffness" });

			layout.SpringStiffness = 999;
			changes.Should().Equal(new string[] { "SpringStiffness", "SpringStiffness" });
		}
		...
		public void TestSpringDampening()
		{
			...
			changes.Should().Equal(new string[] { "SpringDampening" });

			layout.SpringDampening = 1001;
			changes.Should().Equal(new string[] { "SpringDampening" });

			layout.SpringDampening = 999;
			changes.Should().Equal(new string[] { "SpringDampening", "SpringDampening" });
		}
		...
		public void TestDistance()
		{
			...
			changes.Should().Equal(new string[] { "Distance" });

			layout.Distance = 1001;
			changes.Should().Equal(new string[] { "Distance" });

			layout.Distance = 999;
			changes.Should().Equal(new string[] { "Distance", "Distance" });
		}
	}
}