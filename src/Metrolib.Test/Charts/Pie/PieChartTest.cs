...
namespace Metrolib.Test.Charts.Pie
{
	...
	public sealed class PieChartTest
	{
		...
		public void TestLoad1()
		{
			...
			new Action(() => chart.RaiseEvent(new RoutedEventArgs(FrameworkElement.LoadedEvent))).Should().NotThrow("because the control should allow for loading having been unloaded");
		}
		...
		{
			var chart = new PieChart { Series = new PieSeries { Slices = { new PieSlice() } } };
			new Action(() => chart.Arrange(new Rect(0, 0, 128, 128))).Should().NotThrow("because the chart should be arrangeable without having been loaded");
		}
		...
		public void TestArrange2()
		{
			...
			chart.RaiseEvent(new RoutedEventArgs(FrameworkElement.LoadedEvent));
			chart.RaiseEvent(new RoutedEventArgs(FrameworkElement.UnloadedEvent));
			new Action(() => chart.Arrange(new Rect(0, 0, 128, 128))).Should().NotThrow("because the chart should be arrangeable after having been previously unloaded and loaded again");
		}
