...
namespace Metrolib.Test
{
	...
	public sealed class DependencyObjectExtensionsTest
	{
		...
		{
			_grid.FindChildrenOfType<Grid>().Should().Equal(new object[] {_grid});
		}

		[Test]
		public void TestFindChildrenOfType2()
		{
			_grid.FindChildrenOfType<Button>().Should().Equal(new object[] { _button, _flatButton });
			_grid.FindChildrenOfType<FlatButton>().Should().Equal(new object[] { _flatButton });
